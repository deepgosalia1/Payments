...
import EBoardOperators from '../../assets/constants/ElectricBoardOperators'
import Promocodes from '../../assets/constants/promodata'
import IndiaStates from '../../assets/constants/IndianStates'
    ...
    return (
        <View style={{ flex: 1, padding: 10 }}>
            ...
            >
                ...
                <Button style={{ flex: 1, width: '30%', alignSelf: 'center', justifyContent: 'flex-end', marginBottom: 15 }} onPress={() => setStateOverlay(false)}> Close </Button>
            </Overlay>
            ...
            >
                ...
                <View style={{ alignSelf: 'center', height: '70%', width: '90%' }}>
                    <FlatList
                        style={{ marginVertical: 15 }}
                        keyExtractor={(item) => item.id}
                        data={EBoardOperators}
                        renderItem={({ item }) => (
                            <View>
                                <ListItem
                                    activeOpacity={0.99}
                                    onPress={() => {
                                        setEboard(prev => item.name);
            ...
            <Text style={{ fontSize: 17, marginTop: 5 }}>
                Please Select you electricity board
            </Text>

            <TouchableOpacity
                style={{ height: 50, width: '100%', alignSelf: 'center', marginTop: 20 }}
                onPress={() => setStateOverlay(true)}>
                <Text style={{ fontSize: 18, }}> {state ? state : 'Choose State'} </Text>
            ...
            {state && <TouchableOpacity
                ...
                <Text style={{ fontSize: 18, }}> {EBoard ? EBoard : 'Choose Operator'} </Text>
                <Divider style={{ marginTop: 10, borderWidth: 0.2 }} />
            </TouchableOpacity>}
