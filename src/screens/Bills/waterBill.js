...
import WBoardOperators from '../../assets/constants/WaterProviders'
...
export default function WaterBill() {
    const [WBoard, setWboard] = useState();
    ...
    return (
        <View style={{ flex: 1, padding: 10 }}>
            ...
            >
                ...
                <View style={{ alignSelf: 'center', height: '70%', width: '90%' }}>
                    <FlatList
                        ...
                        data={WBoardOperators}
                        renderItem={({ item }) => (
                            <View>
                                <ListItem
                                    activeOpacity={0.99}
                                    onPress={() => {
                                        setWboard(prev => item.name);
            ...
            <Text style={{ fontSize: 17, marginTop: 5 }}>
                Please Select you Water board
            </Text>

            <TouchableOpacity
                style={{ height: 50, width: '100%', alignSelf: 'center', marginTop: 20 }}
                onPress={() => setOverlayVisible(true)}>
                <Text style={{ fontSize: 18, }}> {WBoard ? WBoard : 'Choose Water Board Authority'} </Text>
